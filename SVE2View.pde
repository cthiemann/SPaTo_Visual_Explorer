...
class SVE2View {
  ...
  void draw() {
    ...
    aNodes += 3*((showNodes ? 1 : 0) - aNodes)*min(dt, 1/3.);
    aLinks += 3*((linksVisible ? 1 : 0) - aLinks)*min(dt, 1/3.);
    aSkeleton += 3*((showSkeleton ? 1 : 0) - aSkeleton)*min(dt, 1/3.);
    aNeighbors += 3*((showNeighbors ? 1 : 0) - aNeighbors)*min(dt, 1/3.);
    aNetwork += 3*((showNetwork ? 1 : 0) - aNetwork)*min(dt, 1/3.);
    aLabels += 3*((showLabels ? 1 : 0) - aLabels)*min(dt, 1/3.);
    // get current data
    float[] val = null;
    if (hasData) {
      switch (datatype) {
    ...
    p.setScalingToFitWithin(wrap ? width : .9*width, .9*height);
    if (wrap) {  // FIXME: wrapping should be handled by the projection
      float targetViewWidth = (wrap ? width : .9*width)*zoom[viewMode];  // width of the scaled data (used for wrapping)
      viewWidth += 3*(targetViewWidth - viewWidth)*min(dt, 1/3.);
    ...
    for (int i = 0; i < NN; i++) {
      ...
      float ta = invis ? 0 : 1;
      nodes[i].x = Float.isNaN(nodes[i].x) ? tx : nodes[i].x + 3*(tx - nodes[i].x)*min(dt, 1/3.);
      nodes[i].y = Float.isNaN(nodes[i].y) ? ty : nodes[i].y + 3*(ty - nodes[i].y)*min(dt, 1/3.);
      nodes[i].a += 3*(ta - nodes[i].a)*min(dt, 1/3.);
      if (wrap) {
    ...
    // draw links
    if (hasSlices && ((aLinks > 1/192.) || (aSkeleton > 1/192.) || (aNeighbors > 1/192.) || (aNetwork > 1/192.))) {
      ...
      if (aNeighbors > 1/192.) synchronized (loglinks) {  // sync against SortedLinkList.sort()
      ...
      // show full network
      if (aNetwork > 1/192.) synchronized (loglinks) {  // sync against SortedLinkList.sort()
        for (int l = 0; l < loglinks.NL; l++) {
          int i = loglinks.src[l], j = loglinks.dst[l];
      ...
      if (aLinks > 1/192.) {
        ...
        if (!showLinksWithSkeleton && !showLinksWithNeighbors && !showLinksWithNetwork && !(aSNN > 1 - 1/192.)) aSNN = 0;
        float aN = (showLinksWithNetwork || (aNetwork > 1 - 1/192.)) ? aNetwork : 0;
        ...
        for (int i = 0; i < NN; i++) {
          ...
          if (searchMatchesValid) {
            float alphafactor = (searchMatches[i] || (searchMatchesChild[i] > 0)) ? 1.25 : 0.25;
            stroke(64 + 128*aSNN, 64*(1 - aSNN), 64*(1 - aSNN), min(255, (192*aLinks + 63*aSNN)*alphafactor));
            strokeWeight((alphafactor > 1) ? 1 : 0.25);
          }
          if (!wrap || (abs(nodes[i].x - nodes[j].x) < viewWidth/2))
            line(nodes[i].x, nodes[i].y, nodes[j].x, nodes[j].y);
          else {
    ...
    nodeSize += 3*(nodeSize_target - nodeSize)*min(dt, 1/3.);
    if (aNodes > 1/192.) {
      noStroke();
      for (int i = 0; i < NN; i++) {
        float alphafactor = !searchMatchesValid ? 1 : (searchMatches[i] ? 1.25 : 0.125);
        fill((val == null) ? 127 : colormap.getColor(val[i]), 192*aNodes*nodes[i].a*alphafactor);
        if (fastNodes)
          rect(nodes[i].x + .5, nodes[i].y + .5, 3*nodeSize/4, 3*nodeSize/4);
        else
    ...
    if (r > -1) {
      ...
      if (fastNodes) rect(nodes[r].x, nodes[r].y, 3*nodeSize/4 + .5, 3*nodeSize/4 + .5);
      else ellipse(nodes[r].x, nodes[r].y, nodeSize + .5, nodeSize + .5);
    }
    if (ih > -1) {
      stroke(200, 0, 0); noFill();
      if (fastNodes) rect(nodes[ih].x, nodes[ih].y, 3*nodeSize/4 + .5, 3*nodeSize/4 + .5);
      else ellipse(nodes[ih].x, nodes[ih].y, nodeSize + .5, nodeSize + .5);
    }
    rectMode(CORNER);
    // draw labels
